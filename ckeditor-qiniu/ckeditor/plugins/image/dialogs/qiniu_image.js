...
( function() {
	var imageDialog = function( editor, dialogType ) {
			...
			var IMAGE = 1,
				...
				PREVIEW = 4,
				CLEANUP = 8,
				regexGetSize = /(^\s*(\d+)((px)|\%)?\s*$)|^\s*auto\s*$/i,
				regexGetSizeOrEmpty = /(^\s*(\d+)((px)|\%)?\s*$)|^\s*auto\s*$|^$/i,
				pxLengthRegex = /^\d+px$/;
			...
			var numbering = function( id ) {
				...
				btnAutoFitParentSizeId = numbering( 'btnAutoFitParentSize' ),
			...
			return {
				...
				onLoad: function() {
					...
					if ( this.getContentElement( 'info', 'ratioLock' ) ) {
						...
						this.addFocusable( doc.getById( btnAutoFitParentSizeId ), 5 );
					}
								...
								},
								{
									...
									onLoad: function() {
										...
										var resetButton = CKEDITOR.document.getById( btnResetSizeId ),
											...
											autoFitParentSizeButton = CKEDITOR.document.getById( btnAutoFitParentSizeId) ;
										if ( resetButton ) {
										...
										if ( autoFitParentSizeButton ) {
											...
											autoFitParentSizeButton.on( 'mouseover', function() {
												this.addClass( 'cke_btn_over' );
											}, autoFitParentSizeButton );
											autoFitParentSizeButton.on( 'mouseout', function() {
												this.removeClass( 'cke_btn_over' );
											}, autoFitParentSizeButton );
										}
									...
									html: '<div>' +
										...
										'" class="cke_btn_locked" id="' + btnLockSizesId + '" role="checkbox" style="display:none" ><span class="cke_icon"></span><span class="cke_label">' + editor.lang.image.lockRatio + '</span></a>' +
										'<a href="javascript:void(0)" tabindex="-1" title="' + editor.lang.image.resetSize +
										'" class="cke_btn_reset" id="' + btnResetSizeId + '" role="button"><span class="cke_label">' + editor.lang.image.resetSize + '</span></a>' +
										'<a href="javascript:void(0)" tabindex="-1" title="' + editor.lang.image.autoFitParentSize +
										'" class="cke_btn_auto_fit_parent_size" id="' + btnAutoFitParentSizeId + '" role="button"><span class="cke_label">' + editor.lang.image.autoFitParentSize + '</span></a>' +
										'</div>'
								} ]
							},
							{
